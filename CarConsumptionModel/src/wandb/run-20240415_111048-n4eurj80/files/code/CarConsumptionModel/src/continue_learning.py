...
from utils.callbacks import CustomProgressBarCallback, CustomWandbCallback
...
callback = CallbackList([checkpoint_callback, custom_progress_bar_callback, wandbcallback])
#pretrained_model = A2C.load(f"../models/{args.model_name}.zip", env=env)
pretrained_model = PPOInteraction.load(f"../models/{args.model_name}.zip", 
    env=env,
    verbose=1, 
    **config
)
