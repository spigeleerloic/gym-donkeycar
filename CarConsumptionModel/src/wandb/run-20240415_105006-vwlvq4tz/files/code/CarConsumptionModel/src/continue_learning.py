...
wandbcallback = WandbCallback(gradient_save_freq=100, verbose=2)
...
#pretrained_model = PPO.load(f"../models/{args.model_name}.zip", env=env, verbose=1)
#pretrained_model = PPOInteraction.load(f"../models/{args.model_name}.zip", env=env, verbose=1)

policy = torch.load(f"../models/{args.model_name}.pt")
pretrained_model = PPOInteraction(**config, policy=policy, env=env, verbose=1)
